...
- メッセージ一覧表示に発生にするN+1問題を防ぐため、eager loading(with)を使用して、投稿ユーザー情報をまとめて取得し、パフォーマンスを改善
...
## ER図・画面遷移図
![ER図・画面遷移図](images/er_diagram.png)
- ユーザーは複数のチャンネル・メッセージを持つ
- チャンネルは作成者(users)に紐づく
- メッセージはチャンネル・投稿ユーザーに紐づく
- ログイン後は最後に閲覧したチャンネルへ自動遷移
